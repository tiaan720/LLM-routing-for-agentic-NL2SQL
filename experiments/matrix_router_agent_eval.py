...
def process_single_query(question: str, base_kwargs: Dict) -> Dict:
    """Process a single query using the Matrix router and SQL agent, with retry and tool call extraction"""
    ...
    def stream_and_process():
        nonlocal agent, input_cost, output_cost, model_config
        model_filename = base_kwargs.get("model_filename", "router_model.pkl")
        model_type, model_config = get_routed_model(
            query=question, model_filename=model_filename
        )
        logger.info(f"Routed question to model: {model_config}")
        routed_model = create_model(model=model_type, model_kwargs=model_config)
        # Use the agent_kwargs passed from the main function (remove model_filename key)
        agent_kwargs = {k: v for k, v in base_kwargs.items() if k != "model_filename"}
        ...
        return process_agent_stream_with_store(
            ...
            agent_name="matrix_router_sql_agent",
    ...
    return {
        ...
        "routed_model": model_config.get("model_name", "unknown"),
    }
...
def main(datasets=None, experiment_name_suffix="", model_filename="router_model.pkl"):
    ...
    if len(datasets) == 1:
        exp_name = f"matrix_router_evaluation_{datasets[0]}"
    else:
        exp_name = f"matrix_router_evaluation_multi"

    if experiment_name_suffix:
        exp_name += f"_{experiment_name_suffix}"

    # Create a process function that includes the model_filename
    def process_query_with_model_filename(question: str, base_kwargs: Dict) -> Dict:
        ...
        # Create agent_kwargs with the correct database name and model_filename
        agent_kwargs = {
    ...
    results = run_evaluation(
        ...
        process_query_fn=process_query_with_model_filename,
